...
import { ArrowRight, Sparkles, Target, TrendingUp, Users, Award, ShieldCheck, Wallet } from 'lucide-react';
    ...
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <section className="relative min-h-[90vh] flex items-center pt-32 pb-48 overflow-hidden bg-[#0c4a6e]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
          <div className="flex flex-col items-center text-center">
            <div className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-white/5 backdrop-blur-2xl border border-white/10 text-white text-[10px] font-black uppercase tracking-[0.3em] mb-12 shadow-2xl">
              <Sparkles className="w-3.5 h-3.5 text-brand-500" />
              L'excellence à portée de main
            </div>
            
            <h1 className="text-5xl md:text-[6rem] font-serif font-bold text-white leading-[1.05] mb-10 max-w-5xl tracking-tight drop-shadow-2xl">
              Propulsez votre salon <br /> vers <span className="text-brand-500 italic font-normal">l'excellence</span>
            </h1>
            
            <p className="text-xl md:text-2xl text-slate-200 mb-14 leading-relaxed max-w-3xl font-medium opacity-95 px-4">
            ...
            <div className="flex flex-col sm:flex-row gap-6 w-full sm:w-auto px-6">
              {user ? (
                <>
                  <Link
                    to="/caisse"
                    className="bg-emerald-500 text-white px-14 py-6 rounded-2xl font-black text-xs uppercase tracking-[0.25em] shadow-2xl shadow-emerald-500/20 hover:bg-emerald-600 hover:-translate-y-1 transition-all flex items-center justify-center gap-4 group"
                  >
      ...
      <section className="py-32 bg-[#0c4a6e] text-white relative overflow-hidden">
        <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
          ...
          >
            {user ? "Aller au tableau de bord" : "Commencer le diagnostic gratuit"}
          </Link>
        </div>
      </section>
    </div>
