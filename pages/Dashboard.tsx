...
import { analyzeBusinessTrends, analyzePredictiveBI } from '../services/geminiService';
...
const Dashboard: React.FC = () => {
  ...
  const [predictiveAdvice, setPredictiveAdvice] = useState<string | null>(null);
  ...
  const loadSalonData = async () => {
    ...
    try {
      ...
      if (trans.length > 5 && !isStaff) {
        ...
        if (isElite && !predictiveAdvice) {
           setLoadingPredictive(true);
           const pred = await analyzePredictiveBI(trans, user.firstName || 'Gérant');
           setPredictiveAdvice(pred);
  ...
  const kpis = useMemo(() => {
    ...
    return { avgTicket, expenseRatio, totalCredits };
  }, [salonTransactions]);
    ...
    <div className="min-h-screen bg-slate-50 w-full">
      ...
      <div className="max-w-6xl mx-auto px-6 mt-[-100px] pb-32 space-y-12 relative z-20 w-full">
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
           ...
           <QuickActionBtn icon={<Megaphone className="w-6 h-6" />} label="ANNONCE" sub="Journal Public" onClick={() => navigate('/publier-annonce')} color="bg-brand-900" />
        ...
        {/* SECTION PREDICTIVE BI */}
        {isElite && !isStaff && (
          <section className="bg-slate-900 rounded-[3rem] p-10 md:p-14 shadow-2xl relative overflow-hidden group border border-amber-500/30">
             <div className="absolute top-0 right-0 p-12 opacity-10 rotate-12 transition-transform group-hover:scale-110 duration-1000">
                <BrainCircuit className="w-48 h-48 text-amber-400" />
             </div>
             <div className="relative z-10">
                <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-10">
                   <div className="flex items-center gap-5">
                      <div className="h-16 w-16 bg-amber-400 rounded-3xl flex items-center justify-center shadow-lg shadow-amber-400/20"><Zap className="w-8 h-8 text-brand-900" /></div>
                      <div><h2 className="text-2xl font-serif font-bold text-white tracking-tight">Radar Prédictif IA</h2><p className="text-amber-400/60 font-black text-[9px] uppercase tracking-[0.3em]">Analyse du Futur par Coach Kita</p></div>
                   </div>
                   {predictiveAdvice && (
                      <button onClick={loadSalonData} className="flex items-center gap-2 text-white/40 hover:text-amber-400 transition-colors font-black text-[10px] uppercase tracking-widest">
                         <RefreshCw className={`w-4 h-4 ${loadingPredictive ? 'animate-spin' : ''}`} /> Actualiser Prédictions
                      </button>
                   )}
                </div>
                {loadingPredictive ? (
                   <div className="py-20 text-center space-y-6"><Loader2 className="w-12 h-12 text-amber-400 animate-spin mx-auto" /><p className="text-slate-400 text-sm font-medium animate-pulse">L'IA scanne les cycles de vos clients pour prédire votre semaine...</p></div>
                ) : predictiveAdvice ? (
                   <div className="grid lg:grid-cols-1 gap-8 animate-in fade-in zoom-in-95">
                      <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-[2.5rem]">
                         <div className="prose-kita-dark max-w-none">
                            {predictiveAdvice.split('\n').map((line, i) => {
                               if (line.includes('[MÉTÉO]')) return <div key={i} className="flex items-center gap-4 mb-6"><div className="bg-emerald-500 px-4 py-2 rounded-xl text-[10px] font-black text-white uppercase">{line.replace('[MÉTÉO] :', '')}</div><p className="text-white/60 text-xs font-bold uppercase tracking-widest">Tendance 7 Jours</p></div>;
                               if (line.includes('[CHIFFRE]')) return <div key={i} className="mb-8"><p className="text-amber-400 text-4xl font-black tracking-tighter">{line.replace('[CHIFFRE] :', '')}</p><p className="text-white/40 text-[9px] font-black uppercase tracking-widest mt-1">Opportunité de CA à saisir</p></div>;
                               let processed = line.replace(/\*\*(.*?)\*\*/g, '<strong class="text-amber-400 font-black">$1</strong>');
                               return <p key={i} className="text-slate-300 text-lg leading-relaxed mb-4 italic" dangerouslySetInnerHTML={{ __html: processed }} />;
                            })}
                         </div>
                      </div>
                   </div>
                ) : (
                   <div className="py-20 text-center border-2 border-dashed border-white/10 rounded-[3rem]"><Eye className="w-12 h-12 text-white/10 mx-auto mb-4" /><p className="text-white/30 italic text-sm font-medium">Continuez à enregistrer vos ventes pour débloquer les prédictions.</p></div>
                )}
             </div>
          </section>
        )}
        ...
        {businessInsight && !isStaff && (
          <section className="bg-white rounded-[3rem] p-8 md:p-12 shadow-2xl border-l-[12px] border-emerald-500 animate-in slide-in-from-top-4">
             <div className="flex flex-col md:flex-row gap-8 items-center">
                <div className="h-20 w-20 rounded-full overflow-hidden shrink-0 border-4 border-emerald-50 shadow-xl"><img src={COACH_KITA_AVATAR} className="w-full h-full object-cover" alt="Coach" /></div>
                <div className="flex-grow"><div className="flex items-center gap-2 text-emerald-600 font-black text-[10px] uppercase tracking-widest mb-2"><PlusCircle className="w-3 h-3" /> Analyse Flash du Mentor</div><p className="text-xl font-serif italic text-slate-700 leading-relaxed">"{businessInsight}"</p></div>
        ...
        <div className="grid lg:grid-cols-12 gap-8">
           ...
           <div className="lg:col-span-8 bg-white rounded-[3rem] p-10 shadow-2xl border border-slate-100">
              <div className="flex items-center justify-between mb-8"><div className="flex items-center gap-4"><Target className="w-6 h-6 text-brand-600" /><h3 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em]">Défis du Jour</h3></div></div>
              <div className="space-y-3">
                 {dailyTasks.map((t, idx) => (
                    <button key={idx} onClick={() => {
                       ...
                       <div className={`h-8 w-8 rounded-xl flex items-center justify-center shrink-0 transition-all ${t.completed ? 'bg-emerald-500 text-white' : 'bg-white border'}`}>{t.completed ? <CheckCircle2 className="w-5 h-5" /> : <Circle className="w-5 h-5" />}</div>
        ...
        {!isStaff && (
          <section className="space-y-8 animate-in slide-in-from-bottom-6">
            <div className="flex items-center gap-4 px-4"><BarChart3 className="w-6 h-6 text-brand-600" /><h2 className="text-sm font-black text-slate-400 uppercase tracking-[0.4em]">Pilotage Décisionnel</h2><div className="h-px bg-slate-200 flex-grow"></div></div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
               <KPICard icon={<Receipt className="w-6 h-6" />} label="Ticket Moyen" value={`${kpis.avgTicket.toLocaleString()} F`} sub="CA par client" color="text-indigo-600" bgColor="bg-indigo-50" />
               <KPICard icon={<MapPin className="w-6 h-6" />} label="Présence Google" value={user.gmbStatus === 'ACTIVE' ? "Répertorié" : user.gmbStatus === 'PENDING' ? "En cours..." : "Non visible"} sub={user.gmbStatus === 'ACTIVE' ? "Votre salon est sur Maps" : "Attirer de nouveaux clients"} color={user.gmbStatus === 'ACTIVE' ? "text-emerald-600" : "text-amber-600"} bgColor={user.gmbStatus === 'ACTIVE' ? "bg-emerald-50" : "bg-amber-50"} onClick={() => navigate('/gmb-expert')} />
               <KPICard icon={<AlertTriangle className="w-6 h-6" />} label="Dettes Clients" value={`${kpis.totalCredits.toLocaleString()} F`} sub="Total à recouvrer" color={kpis.totalCredits > 0 ? "text-rose-600" : "text-slate-400"} bgColor="bg-rose-50" onClick={() => navigate('/caisse')} alert={kpis.totalCredits > 10000} />
               <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100 flex flex-col justify-between group">
                  <div>
                    <div className="flex justify-between items-start mb-4"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Charges / CA</p><TrendingUp className="w-5 h-5 text-amber-500" /></div>
                  ...
                  <div className="mt-6 h-2 w-full bg-slate-100 rounded-full overflow-hidden"><div className={`h-full transition-all duration-1000 ${kpis.expenseRatio > 40 ? 'bg-rose-500' : 'bg-emerald-500'}`} style={{ width: `${kpis.expenseRatio}%` }}></div></div>
               </div>
            </div>
          </section>
        )}

        {/* SECTION MES ENGAGEMENTS */}
        <section className="bg-white rounded-[4rem] p-10 md:p-14 shadow-2xl border border-slate-100">
           <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-12">
              <div className="flex items-center gap-5">
                 <div className="h-16 w-16 bg-brand-50 text-brand-600 rounded-[1.5rem] flex items-center justify-center shadow-inner"><Target className="w-8 h-8" /></div>
                 <div><h2 className="text-2xl font-black text-slate-900 uppercase tracking-tight">Mes Engagements</h2><p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1">Actions scellées avec le Mentor</p></div>
              </div>
              {user.actionPlan && user.actionPlan.length > 0 && (
                <div className="bg-emerald-50 px-6 py-3 rounded-2xl border border-emerald-100 flex items-center gap-3">
                   ...
                   <span className="text-[10px] font-black text-emerald-700 uppercase tracking-widest">{user.actionPlan.filter(a => a.isCompleted).length} / {user.actionPlan.length} Réalisés</span>
                </div>
              )}
           </div>
           {user.actionPlan && user.actionPlan.length > 0 ? (
             <div className="grid md:grid-cols-2 gap-6">
                {user.actionPlan.map((commitment, idx) => {
                   ...
                   return (
                      <div key={idx} className={`p-8 rounded-[2.5rem] border-2 transition-all relative group ${commitment.isCompleted ? 'bg-emerald-50/30 border-emerald-100 shadow-sm' : 'bg-white border-slate-50 shadow-xl hover:border-brand-100'}`}>
                         <div className="flex justify-between items-start mb-6">
                            <div className="flex items-center gap-3"><div className={`p-2 rounded-lg ${commitment.isCompleted ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-400'}`}><Book className="w-3.5 h-3.5" /></div><span className="text-[9px] font-black text-slate-400 uppercase tracking-widest line-clamp-1 max-w-[150px]">{commitment.moduleTitle}</span></div>
                            <span className="text-[8px] font-bold text-slate-300 uppercase tracking-tighter">{commitment.date}</span>
                         </div>
                         <p className={`text-lg font-bold leading-relaxed mb-8 ${commitment.isCompleted ? 'text-slate-400 line-through italic' : 'text-slate-900'}`}>"{commitment.action}"</p>
                         <button onClick={() => handleToggleCommitment(commitment)} disabled={isUpdating} className={`w-full py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-3 transition-all ${commitment.isCompleted ? 'bg-white text-emerald-600 border border-emerald-100 shadow-sm' : 'bg-brand-900 text-white shadow-xl hover:bg-brand-950'}`}>
             ...
             <div className="py-20 text-center border-2 border-dashed border-slate-100 rounded-[3rem] group hover:border-brand-200 transition-colors"><div className="h-20 w-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-6 text-slate-300 group-hover:scale-110 transition-transform"><ClipboardList className="w-10 h-10" /></div><h3 className="text-xl font-bold text-slate-400 mb-2 italic">"Aucun engagement scellé pour le moment."</h3><button onClick={() => navigate('/mes-formations')} className="bg-brand-50 text-brand-600 px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-brand-100 transition-all">Aller aux formations</button></div>
           )}
        </section>
      </div>
    </div>
...
const KPICard = ({ icon, label, value, sub, color, bgColor, onClick, alert }: any) => (
  <button onClick={onClick} disabled={!onClick} className={`bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100 text-left flex flex-col justify-between group transition-all ${onClick ? 'hover:scale-[1.02] active:scale-95' : 'cursor-default'} ${alert ? 'ring-2 ring-rose-500/20' : ''}`}>
    <div className={`p-3 rounded-2xl w-fit mb-6 transition-transform group-hover:rotate-12 ${bgColor} ${color}`}>{icon}</div>
    <div>
