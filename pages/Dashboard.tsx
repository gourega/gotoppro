...
import { DAILY_CHALLENGES, TRAINING_CATALOG, BADGES, LEGACY_ID_MAP, COACH_KITA_AVATAR, DIAGNOSTIC_QUESTIONS } from '../constants';
  ...
  return (
    <div className="min-h-screen bg-slate-50 w-full">
              ...
              <div className={`inline-flex items-center gap-4 px-6 py-3 rounded-2xl border backdrop-blur-md transition-all ${isElite ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-amber-500/10 border-amber-500/30 text-amber-400'}`}>
                 ...
                 <div className="flex flex-col">
                    ...
                    <span className="text-[8px] font-bold opacity-70">
                      {isElite ? 'Données certifiées et synchronisées' : 'Données non sauvegardées en ligne'}
                    </span>
                 </div>
              </div>
            )}
      ...
      <div className="max-w-6xl mx-auto px-6 mt-12 pb-32 space-y-12 relative z-20 w-full">
        ...
        {/* SECTION : VISION STRATÉGIQUE (DYNAMIQUE) */}
           ...
           <div className="flex flex-col md:flex-row gap-10 items-start relative z-10">
              ...
              <div className="flex-grow">
                 <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    ...
                    {user.strategicAudit && (
                      <button onClick={handleGenerateAudit} disabled={loadingAudit} className="text-[9px] font-black text-slate-400 uppercase tracking-widest hover:text-indigo-600 transition-colors flex items-center gap-2">
                         {loadingAudit ? <Loader2 className="animate-spin w-3 h-3" /> : <RefreshCw className="w-3 h-3" />} Mettre à jour mon audit
                      </button>
                    )}
                 </div>
                 
                 {user.strategicAudit ? (
                   <div className="prose-kita whitespace-pre-wrap font-medium" 
                   ...
                   <div className="py-10 text-center md:text-left">
                      ...
                      <button 
                        ...
                        className="bg-indigo-600 text-white px-10 py-5 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-xl flex items-center gap-4 mx-auto md:mx-0"
                      >
                         {loadingAudit ? <Loader2 className="animate-spin w-4 h-4" /> : <FileSearch className="w-4 h-4" />}
                         {loadingAudit ? 'Analyse de votre salon...' : 'Générer mon audit stratégique'}
                      </button>
                   </div>
                 )}
              </div>
              ...
              <div className="relative z-10 h-full flex flex-col">
                 ...
                 <p className={`text-sm leading-relaxed mb-10 flex-grow ${isStockExpert ? 'text-slate-400' : 'text-slate-500'}`}>
                    {isStockExpert 
                      ...
                      : 'Augmentez votre marge de 20% en contrôlant vos mélanges et vos stocks de revente.'}
                 </p>
                 {isStockExpert ? (
                    <button onClick={() => navigate('/magasin')} className="w-full bg-sky-500 text-white py-5 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl hover:bg-sky-400 transition-all flex items-center justify-center gap-3">
                       Accéder au Magasin <ArrowRight className="w-4 h-4" />
                    </button>
                 ) : (
              ...
              <div className="relative z-10 h-full flex flex-col">
                 ...
                 <p className="text-slate-500 text-sm leading-relaxed mb-10 flex-grow">
                    {isPerformance 
                      ...
                      : 'Gérez les commissions avec transparence et piloter vos clients VIP.'}
                 </p>
                 {isPerformance ? (
