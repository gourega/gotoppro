...
import { 
  ...
  Sparkles,
  Zap,
  // Removed CrystalBall as it is not exported by lucide-react
  BrainCircuit,
  Eye
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-50 w-full">
      ...
      <div className="max-w-6xl mx-auto px-6 mt-[-100px] pb-32 space-y-12 relative z-20 w-full">
        ...
        {/* SECTION PREDICTIVE BI (Radar du Futur) - EXCLUSIF ELITE */}
        {isElite && !isStaff && (
             ...
             <div className="relative z-10">
                <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-10">
                   <div className="flex items-center gap-5">
                      <div className="h-16 w-16 bg-amber-400 rounded-3xl flex items-center justify-center shadow-lg shadow-amber-400/20">
                         <Zap className="w-8 h-8 text-brand-900" />
                      </div>
                      <div>
                         <h2 className="text-2xl font-serif font-bold text-white tracking-tight">Radar Prédictif IA</h2>
                         <p className="text-amber-400/60 font-black text-[9px] uppercase tracking-[0.3em]">Analyse du Futur par Coach Kita</p>
                      </div>
                   </div>
                   {predictiveAdvice && (
                ...
                {loadingPredictive ? (
                   <div className="py-20 text-center space-y-6">
                      ...
                      <p className="text-slate-400 text-sm font-medium animate-pulse">L'IA scanne les cycles de vos clients pour prédire votre semaine...</p>
                   </div>
                ) : predictiveAdvice ? (
                   ...
                   <div className="py-20 text-center border-2 border-dashed border-white/10 rounded-[3rem]">
                      ...
                      <p className="text-white/30 italic text-sm font-medium">Continuez à enregistrer vos ventes pour débloquer les prédictions.</p>
                   </div>
                )}
             </div>
          </section>
        ...
        {/* ANALYSE PROACTIVE DU MENTOR (Basée sur l'historique) */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
               ...
               <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100 flex flex-col justify-between group">
                  ...
                  <div className="mt-6 h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                    <div className={`h-full transition-all duration-1000 ${kpis.expenseRatio > 40 ? 'bg-rose-500' : 'bg-emerald-500'}`} style={{ width: `${kpis.expenseRatio}%` }}></div>
                  </div>
               </div>
            </div>
          </section>
        ...
        <section className="bg-white rounded-[4rem] p-10 md:p-14 shadow-2xl border border-slate-100">
           ...
           {user.actionPlan && user.actionPlan.length > 0 ? (
                      ...
                      <div key={idx} className={`p-8 rounded-[2.5rem] border-2 transition-all relative group ${commitment.isCompleted ? 'bg-emerald-50/30 border-emerald-100 shadow-sm' : 'bg-white border-slate-50 shadow-xl hover:border-brand-100'}`}>
                         <div className="flex justify-between items-start mb-6">
                            <div className="flex items-center gap-3">
                               <div className={`p-2 rounded-lg ${commitment.isCompleted ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-400'}`}><Book className="w-3.5 h-3.5" /></div>
                               <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest line-clamp-1 max-w-[150px]">{commitment.moduleTitle}</span>
                            </div>
             ...
             <div className="py-20 text-center border-2 border-dashed border-slate-100 rounded-[3rem] group hover:border-brand-200 transition-colors">
                <div className="h-20 w-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-6 text-slate-300 group-hover:scale-110 transition-transform"><ClipboardList className="w-10 h-10" /></div>
                <h3 className="text-xl font-bold text-slate-400 mb-2 italic">"Aucun engagement scellé pour le moment."</h3>
                <button onClick={() => navigate('/mes-formations')} className="bg-brand-50 text-brand-600 px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-brand-100 transition-all">Aller aux formations</button>
             </div>
           )}
        </section>
        ...
        <section className="bg-white rounded-[4rem] p-10 md:p-14 shadow-2xl border-t-[8px] border-amber-400 relative overflow-hidden group w-full">
           <div className="flex flex-col md:flex-row justify-between items-center gap-12 relative z-10">
              <div className="space-y-6 text-center md:text-left">
                 ...
                 <p className="text-slate-500 font-medium max-w-md">{isStaff ? "Suivez vos commissions et pourboires en temps réel pour mesurer votre montée en gamme." : "Suivez vos recettes, vos dépenses et recouvrez vos ardoises pour garantir la santé financière de votre empire."}</p>
                 <div className="flex gap-4 justify-center md:justify-start">
                    <button onClick={() => navigate('/caisse')} className="bg-brand-900 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl hover:bg-brand-950 transition-all flex items-center gap-3">{isStaff ? 'Journal de bord' : 'Ouvrir la Caisse'} <ArrowRight className="w-4 h-4 text-amber-400" /></button>
                    {!isStaff && (
                      <button onClick={() => navigate('/caisse?mode=expense')} className="bg-rose-50 text-rose-600 px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest border border-rose-100 shadow-sm hover:bg-rose-100 transition-all flex items-center gap-3"><MinusCircle className="w-4 h-4" /> Dépense</button>
                    )}
                 </div>
              </div>
